...
const QString title = "PassManager";
...
WindowFrame::WindowFrame(QWidget *parent, QWidget *child)
    : QFrame(parent), mChild(child), ui(new Ui::WindowFrame){
...
void WindowFrame::exit(){
    if (mTrayIcon) {
            mTrayIcon->hide();  // Убираем значок из трея
        }
    close();  // Закрываем основное окно
...
void WindowFrame::initTrayMenu(){
    ...
    mTrayIcon->setIcon(QIcon(appIcon));
